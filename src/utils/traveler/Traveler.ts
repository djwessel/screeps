...
export class Traveler {
  ...
  public static addStructuresToMatrix(room: Room, matrix: CostMatrix, roadCost: number): CostMatrix {
    ...
    for (let site of room.find<FIND_MY_CONSTRUCTION_SITES>(FIND_MY_CONSTRUCTION_SITES)) {
      if (
  ...
  public static addCreepsToMatrix(room: Room, matrix: CostMatrix): CostMatrix {
    room.find<FIND_CREEPS>(FIND_CREEPS).forEach((creep: Creep) => matrix.set(creep.pos.x, creep.pos.y, 0xff));
    return matrix;
  }
